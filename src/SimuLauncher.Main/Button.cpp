...
void Button::handleEvent(SDL_Event& e, int xMouse, int yMouse) //dans le repere du button
{
	if (xMouse >= 0 && xMouse <= (this->getW()) && yMouse >= 0 && yMouse <= (this->getH()))
	{
		...
		if (e.type == SDL_MOUSEBUTTONDOWN)
		{
			...
			{
				if (listener_ != NULL) listener_->leftButtonPressed();
				pressleft_ = true;
			}
			if (e.button.button == SDL_BUTTON_RIGHT)
			{
				if (listener_ != NULL) listener_->rightButtonPressed();
				pressright_ = true;
			}
		}
		if (e.type == SDL_MOUSEBUTTONUP)
		{
			if (e.button.button == SDL_BUTTON_LEFT)
			{
				if (pressleft_)
				{
					if (listener_ != NULL) listener_->leftButtonPressedAndRelease();
					pressleft_ = false;
				}
			}
			if (e.button.button == SDL_BUTTON_RIGHT)
			{
				if (pressright_)
				{
					if (listener_ != NULL) listener_->rightButtonPressedAndRelease();
					pressright_ = false;
				}
			}
		}
...
bool Button::loadMediaButton(SDL_Renderer * renderer, std::string pathMouseOut,
		std::string pathMouseOver, std::string pathMouseDown, std::string pathMouseUp)
{

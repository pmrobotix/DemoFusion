...
LedBar::LedBar(std::string botId, Actions & actions, int nbLed) :
		AActionsElement(actions), nbLed_(nbLed), actionStopped_(false), actionRunning_(
				false), position_(0), color_(0), nb_(0), timeus_(0), hex_(0), hexNext_(
				0)
...
LedBarAction::LedBarAction(LedBar & ledBar, LedBarActionName action) :
		ledBar_(ledBar), action_(action), lastTime_(0), i_(1), j_(1), k_(0), inc_(
				true)
{
	chrono_.start();
}
...
void LedBar::startAlternate(uint nb, uint timeus, uint hexValue,
		uint hexValueNext, uint color, bool wait)
{
...
void LedBar::startBlinkPin(uint nb, uint timeus, int position, uint color,
		bool wait)
{
	while (this->actionRunning_)
	{
...
bool LedBarAction::execute()
{
	...
	case LEDBARALTERNATE:
		...
		if (chrono_.getElapsedTimeInMicroSec()
				>= lastTime_ + (long) ledBar_.timeus() || i_ == 1)
		{
			if ((i_ % 2))
			{
				ledBar_.flash(ledBar_.hexValue(), ledBar_.color());
